/*
	File: fn_moduleEffectsSmokeCore.sqf
	Author: Borivoj Hlava (tiny customization changes by SaOk)

	Description:
	Module function. Creates smoke on position of module - core part.
...
_particleLifeTime = _logic getVariable ["ParticleLifeTime",10+(random 10)];
_particleDensity = _logic getVariable ["ParticleDensity",5+(random 5)];
_particleSize = _logic getVariable ["ParticleSize",2+(random 5)];
_particleSpeed = _logic getVariable ["ParticleSpeed",0.2+(random 0.3)];
_particleLifting = _logic getVariable ["ParticleLifting",1];
_windEffect = _logic getVariable ["WindEffect",1];
_effectSize = _logic getVariable ["EffectSize",1+(random 0.5)];
...
//--- timeout
	[_logic,_timeout] spawn {
		...
		{deleteVehicle _x;} foreach _emit;
