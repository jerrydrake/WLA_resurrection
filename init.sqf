...
[TEST_MODE,_boatinspos] SPAWN {
	...
	if (isNil"InsT" || {InsT == "BOAT"}) then {
		...
		{_x moveincargo _obj;} foreach _u;
		...
		if (isNil"IFENABLED") then {
			...
			if (!isNil"SPECOPSENABKLED") then {_ra = ["B_mas_it_CH_47F","B_mas_it_Heli_light_03_F","B_mas_it_Heli_Transport_02_F","B_mas_it_Heli_light_03_unarmed_F","B_mas_it_Heli_Light_01_F"];};
