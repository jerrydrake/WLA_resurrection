...
// LEGD = 0; //Unused -Gritt
TeDam = 1; //Unsure what this is fully. Commenting out to see if it makes any changes.
NOTASKNOTI = true;
SAOKDIEWISH = 0;
SAOKTTTEE = [];
SAOKTASKTEE = [];
SAOKHINTTEE = [];
SAOKPIPM = 0;
SAOKPIPP = ["LeftShoulder", 0]; //Possibly related to team PiP mode.
CARRYZONES = [];
NOMORDERED = [];
ARTYZONES = [];
ZONETOMOVE = [];
...
LCON1 = [];
TEST_MODE = false;
SaOkNoAnimals = true; //disabled by default
CapturedAll = 0;
NOMOVEZONES = [];
DONTSTOREZONES = [];
SAOKDYNTASKS = [
    ["SAOKENDT"],
    ["maintasks\captureofficer.sqf"],
    ["SAOKPIERT"]
];
UNITSTOSUR = [];
SZONES = [];
...
ACTIVEZ = [];
PSHOT = 0;
if (productVersion select 2 > 132) then { //Likely for the heli exp. pack? -Gritt
...
_id = addMissionEventHandler["loaded", {
    _n = [] SPAWN GUIcode;
}];
_n = [] SPAWN GUIcode;
...
// I believe this is for the cutscene - Gritt
MORTARAR = [
    ["B_Mortar_01_F"],
    ["O_Mortar_01_F"],
    ["I_Mortar_01_F"],
...
if (worldname != "Altis") then {
    ...
    _k = []; {
        if (count FacMarkers >= _foreachIndex) then {
            _k set[count _k, (FacMarkers select _foreachIndex)];
            (FacMarkers select _foreachIndex) setmarkerpos _x;
            _vil = createLocation["NameLocal", [(_x select 0), (_x select 1) - 3, 0], 100, 100];
            _vil setText "factory";
        } else {
            [_x, "Fac", ""] CALL SAOKCREATESTPOINT;
        };
    }
    foreach(_dat select 0); {
        if !(_x in _k) then {
            FacMarkers = FacMarkers - [_x];
            deletemarker _x;
        };
    }
    foreach FacMarkers;
    _k = []; {
        if (count StoMarkers >= _foreachIndex) then {
            _k set[count _k, (StoMarkers select _foreachIndex)];
            (StoMarkers select _foreachIndex) setmarkerpos _x;
        } else {
            [_x, "Sto", ""] CALL SAOKCREATESTPOINT;
        };
    }
    foreach(_dat select 1); {
        if !(_x in _k) then {
            StoMarkers = StoMarkers - [_x];
            deletemarker _x;
        };
    }
    foreach StoMarkers;
    _k = []; {
        if (count PierMarkers >= _foreachIndex) then {
            _k set[count _k, (PierMarkers select _foreachIndex)];
            (PierMarkers select _foreachIndex) setmarkerpos _x;
        };
    }
    foreach(_dat select 2); {
        if !(_x in _k) then {
            PierMarkers = PierMarkers - [_x];
            deletemarker _x;
        };
    }
    foreach PierMarkers;
    if !(_isWater) then {
        {
            _x setmarkertype "loc_BusStop";
        }
        foreach PierMarkers;
    };
    _k = []; {
        if (count PowMarkers >= _foreachIndex) then {
            _k set[count _k, (PowMarkers select _foreachIndex)];
            (PowMarkers select _foreachIndex) setmarkerpos _x;
        };
    }
    foreach(_dat select 3); {
        if !(_x in _k) then {
            PowMarkers = PowMarkers - [_x];
            deletemarker _x;
        };
    }
    foreach PowMarkers;
    _k = []; {
        if (count AIRFIELDLOCATIONS >= _foreachIndex) then {
            _k set[count _k, (AIRFIELDLOCATIONS select _foreachIndex)];
            (AIRFIELDLOCATIONS select _foreachIndex) setmarkerpos _x;
        };
    }
    foreach(_dat select 4); {
        if !(_x in _k) then {
            AIRFIELDLOCATIONS = AIRFIELDLOCATIONS - [_x];
            deletemarker _x;
        };
    }
    foreach AIRFIELDLOCATIONS;
...
[] SPAWN {
    {
        if (text _x == "factory") then {
            FACTORYLOCATIONS set[count FACTORYLOCATIONS, _x];
        };
    }
    foreach(nearestLocations[getPosATL player, ["NameLocal"], 20000]);
};
...
[TEST_MODE, _boatinspos] SPAWN { //SPAWN OPTIONS START
    ...
    while {
        surfaceIsWater _start || {
            {
                _start distance _x < 800
            }
            count ISLANDPOSES > 0
        }
    }
    ...
    "StartPoint"
    setmarkerpos _start;
    if (count _boatinspos > 0) then {
        "StartPointSea"
        setmarkerpos(_boatinspos call RETURNRANDOM);
    } else {
        "StartPointSea"
        setmarkerpos[0, 0, 0];
    };
    ...
    if ("MetResContact" in (missionnamespace getvariable "Progress")) then {
        _nul = ["task2", "SUCCEEDED", false] call SAOKCOTASK;
    };
    ...
    if (!isNil "PlayerPos") then {
        ...
        _rad = 3000;
        while {
            surfaceiswater PlayerPos
        }
        do {
            PlayerPos = [(_pPos select 0) + _rad - (random _rad) * 2, (_pPos select 1) + _rad - (random _rad) * 2, 0];
            _rad = _rad + 1000;
        };
    ...
    foreach units group player; //Espionage mode
    if (!isNil "CIVMODE" || {
            (!isNil "PlayerPos")
        } || {
            (!isNil "SAOKRESUME")
        ...
        sleep 15;
        _lahin = [0, 0, 0]; {
            if (player distance position _x < player distance _lahin) then {
                _lahin = position _x;
            };
        }
        ...
        if !(["task2"] call BIS_fnc_taskCompleted) then {
            ...
            _someId SPAWN {
                waitUntil {
                    sleep 1;
                    ["task2"] call BIS_fnc_taskCompleted
                };
            ...
            sleep 5;
            "Following the task waypoint isnt often mandatory"
            SPAWN HINTSAOK;
        };
    };
    SAbing = true;
    if (isNil "InsT" || { //Boat
        ...
        sleep 60;
        "Insertion"
        SPAWN SAOKCHAPTERADD;
        [player, player, "PlaV", "V1"] SPAWN SAOKKBTELL;
        [((name player) + ": Get ready, boys. Make sure you stay alert, anything can happen and don't get cocky just because we have a Blackfoot team helping us to secure the LZ."), 10] SPAWN SAOKTITLETEXT;
        waitUntil {
            sleep 5;
            vehicle player == player && {!surfaceisWater getposATL player
            } && {
                {
                    vehicle player distance _x < 200
                }
                count ISLANDPOSES == 0
            }
        };
        ...
        _n = [
            ...
            [
                [
                    [
                        [player], "Truck with constructing gear should be dropped for us at any moment.", 8
                    ],
                    [player, player, "PlaV", _v]
                ]
            ]
        ] SPAWN SAOKCUTSCENE;
        waitUntil {
        ...
        if (!isNil "IFENABLED") then {
            _r = ["Lib_sdkfz251"] call RETURNRANDOM;
        };
        ...
        if (isNil "IFENABLED") then {
            ...
            if (isClass(configFile >> "cfgVehicles" >> "I_Heli_light_03_F")) then {
                _ra = _ra + ["I_Heli_light_03_F", "I_Heli_light_03_unarmed_F"];
            };
            if (!isNil "Eridanus") then {
                _ra = ["TEI_Pelican_unarmed_tan", "TEI_Pelican_unarmed_green", "TEI_Pelican_unarmed_black"];
            };
            if (!isNil "RHSENABLED") then {
                _ra = ["RHS_UH60M_d", "RHS_UH60M", "RHS_CH_47F_light", "RHS_CH_47F"];
            };
            ...
            (_chop select 0) addWeaponCargo["Rangefinder", 1];
            (_chop select 0) addWeaponCargo["Laserdesignator", 1];
            (_chop select 0) addmagazineCargo["Laserbatteries", 1]; {
                _x moveincargo(_chop select 0);
            }
            ...
            _max = 0;
            while {
                ((surfaceNormal _start) select 2 < 0.98 || {
                    [_start, _ra] CALL SAOKTAKENBOX
                }) && {
                    _max < 10
                }
            }
        ...
        [(_chop select 2), (_chop select 0), _start] SPAWN {
            ...
            if (!isNil "RHSENABLED") then {
                _class = ["rhs_ah64d_wd", "rhs_ah64d"];
            };
            if (!isNil "IFENABLED") then {
                _class = ["LIB_P47"];
            };
            if (!isNil "CheConf") then {
                _class = ["I_mas_cer_MI24"];
            };
            ...
            if (isNil "IFENABLED") then {
                ...
                waitUntil {
                    sleep 1;
                    unitready leader(_this select 1) || {!(player in (_this select 1))
                    }
                };
                [player, player, "PlaV", "V1"] SPAWN SAOKKBTELL;
                "Insertion"
                ...
                waitUntil {
                    sleep 1;
                    (getposATL(_this select 1) select 2) < 6 || {!(player in (_this select 1))
                    }
                };
                ...
                waitUntil {
                    sleep 1;
                    (getposATL(_this select 1) select 2) < 1.5 || {!(player in (_this select 1))
                    }
                }; {
                ...
                if ({
                    ...
                    count(units group player) > 0) then {
                    "Dismount your team by pressing 4-1, after selecting the units with F1..F10"
                    SPAWN HINTSAOK;
                };
                waitUntil {
                    sleep 2; {
...
if (isClass(configFile >> "cfgVehicles" >> "AV_IndUs_AR")) then {
    FRIENDC1 = ["AV_IndUs_medic", "AV_IndUs_REP", "AV_IndUs_EXP", "AV_IndUs_AT", "AV_IndUs_Rifleman2", "AV_IndUs_Rifleman", "AV_IndUs_Rifleman_light", "AV_IndUs_SL"];
};
...
player addEventHandler["FiRed", {
    [_this select 5, _this select 2] SPAWN FSmokeSignal;
}];
...
_nul = [] SPAWN {
    ...
    do {
        ...
        if (!isNil "debugi") then {
            hint format["Null objects removed: %1", _nyll];
        };
    };
};
...
if (!isClass(configFile >> "CfgLocationTypes" >> "CampB")) then {
    [] SPAWN {
        ...
        do {
            "MISSION REQUIRES THE LATEST WLA SOUND ADDON VERSION TO PLAY"
            SPAWN HINTSAOK;
            sleep 6;
            "SUBSCRIBE IT FROM WORKSHOP AND ACTIVATE IN ARMA3 LAUNCHER"
            SPAWN HINTSAOK;
            sleep 6;
        };
    };
...
do {
    if (!isNil {
            missionnamespace getvariable "Autosave"
        } && {
            ((missionnamespace getvariable "Autosave") * 60 + _time) < time
        }) then {
        _time = time;
        savegame;
        [] SPAWN FSaOkSave;
    ...
    _c = ({
            _x in (missionnamespace getvariable "Progress")
        }
        count["GreenHelp", "USHelp", "ResHelp"]) * 0.3;
    if (random 1 < _c) then {
        //CSAT FROM PIERS
        [] SPAWN SAOKMORECSAT;
    };
    ...
    5 SPAWN InitFuncs; {
        if (isNil {
                _x getvariable "WithAlarm"
            } && {
                typeof _x in _veAR
            }) then {
            _nul = [_x] SPAWN CarAlarm;
        };
    }
    foreach((getposATL vehicle player) nearEntities[["Car"], 200]);
    sleep 10;
    [] SPAWN FUNKTIO_Commander;
    6 SPAWN InitFuncs;
    5 SPAWN InitFuncs;
