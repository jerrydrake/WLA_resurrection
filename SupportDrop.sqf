...
if (_class == "I_G_Van_01_transport_F") then {_roina setvariable ["Minetruck",1];};
_roina setdir (random 360);
_rrr = [0,0,0];
if (_class iskindof "ReammoBox_F") then {_rrr = [0,0,-1];};
_roina attachTo [_veh,_rrr];
_roina setvariable ["AmCrate",1];
if (_class iskindof "LandVehicle") then {AddIdVeh pushBack _roina;};
[_roina] SPAWN {
...
[_mar2,_mar3] SPAWN {sleep 900;{deletemarker _x;} foreach _this;};
};
