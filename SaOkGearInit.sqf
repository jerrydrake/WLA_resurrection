...
if (_change > 0) then {
_f = false;
{if (_w == _x select 0 || {isClass(configFile >> "cfgWeapons" >> _w) && {((gettext (configfile >> "CfgWeapons" >> _w >> "displayName")) == (gettext (configfile >> "CfgWeapons" >> (_x select 0) >> "displayName")))}}) exitWith {_Ar set [_forEachIndex,[(_x select 0),((_x select 1)+_change)]];_f = true;};} foreach _Ar;
...
switch (_mode) do {
case "F": {
if (VariSelectedUnit canAddItemToUniform _item) then {"" SPAWN HINTSAOK;} else {"NO ROOM TO ADD THE ITEM" SPAWN HINTSAOK;};
...
if (isClass(configFile >> "cfgMagazines" >> _item)) then {_m = {_item == _x} count (magazines VariSelectedUnit); VariSelectedUnit AddItemToUniform _item;if (_m != {_item == _x} count (magazines VariSelectedUnit)) then {_item CALL _f;};}; 
};
case "B": {
if (VariSelectedUnit canAddItemToBackpack _item) then {"" SPAWN HINTSAOK;} else {"NO ROOM TO ADD THE ITEM" SPAWN HINTSAOK;};
...
if (isClass(configFile >> "cfgMagazines" >> _item)) then {_m = {_item == _x} count (magazines VariSelectedUnit); VariSelectedUnit AddItemToBackpack _item;if (_m != {_item == _x} count (magazines VariSelectedUnit)) then {_item CALL _f;};}; 
};
case "V": {
if (VariSelectedUnit canAddItemToVest _item) then {"" SPAWN HINTSAOK;} else {"NO ROOM TO ADD THE ITEM" SPAWN HINTSAOK;};
...
if (isClass(configFile >> "cfgMagazines" >> _item)) then {_m = {_item == _x} count (magazines VariSelectedUnit); VariSelectedUnit AddItemToVest _item;if (_m != {_item == _x} count (magazines VariSelectedUnit)) then {_item CALL _f;};}; 
};
};
};
...
SAOKGEARWMAGS = {
private ["_h","_helpD2","_ctrl","_myDisplay"];
if (typename _this != "STRING") exitWith {};
...
_ll = [];
if (isClass(configFile >> "cfgMagazines" >> "SUPER_flash")) then {_ll pushback "SUPER_flash";};
{
lbAdd [1501, getText(configfile >> "CfgMagazines" >> _x >> "displayName")];
_ctrl lbSetPicture [(_foreachindex+_h),gettext (configfile >> "CfgMagazines" >> _x >> "picture")];
_helpD2 set [count _helpD2, [_x,getText(configfile >> "CfgMagazines" >> _x >> "displayName")]];
} foreach (["Laserbatteries","1Rnd_HE_Grenade_shell","3Rnd_HE_Grenade_shell","HandGrenade","APERSBoundingMine_Range_Mag","APERSTripMine_Wire_Mag","APERSMine_Range_Mag","SLAMDirectionalMine_Wire_Mag","SatchelCharge_Remote_Mag","DemoCharge_Remote_Mag","ClaymoredirectionalMine_Remote_Mag","ATMine_Range_Mag","1Rnd_Smoke_Grenade_shell","SmokeShell","UGL_FlareGreen_F","3Rnd_SmokeYellow_Grenade_shell"]+_ll);
...
SAOKGEARWSHOP = {
private ["_h","_ctrl2","_ctrl","_ok","_myDisplay","_Wpic","_item","_helpD","_helpD2","_name","_names"];
...
{
lbAdd [1501, getText(configfile >> "CfgMagazines" >> _x >> "displayName")];
_ctrl2 lbSetPicture [(_foreachindex+_h),gettext (configfile >> "CfgMagazines" >> _x >> "picture")];
_helpD2 set [count _helpD2, [_x,getText(configfile >> "CfgMagazines" >> _x >> "displayName")]];
} foreach ["Laserbatteries","1Rnd_HE_Grenade_shell","HandGrenade","APERSBoundingMine_Range_Mag","APERSTripMine_Wire_Mag","APERSMine_Range_Mag","SLAMDirectionalMine_Wire_Mag","SatchelCharge_Remote_Mag","DemoCharge_Remote_Mag","ClaymoredirectionalMine_Remote_Mag","ATMine_Range_Mag","1Rnd_Smoke_Grenade_shell","SmokeShell","UGL_FlareGreen_F"];
missionNameSpace setvariable ["helpD2",_helpD2];
lbSetCurSel [1501, 0];
lbAdd [1502, "1"];
lbAdd [1502, "2"];
...
//NEW
_con = getWeaponCargo (uniformContainer VariSelectedUnit);
_cW = + (_con select 0);
_c = count _cW - 1;
_pii = 0;
for "_i" from 0 to _c do {
...
//NEW
_con = getWeaponCargo (backpackContainer VariSelectedUnit);
_cW = + (_con select 0);
_c = count _cW - 1;
_pii = 0;
for "_i" from 0 to _c do {
...
//NEW
_con = getWeaponCargo (vestContainer VariSelectedUnit);
_cW = + (_con select 0);
_c = count _cW - 1;
_pii = 0;
for "_i" from 0 to _c do {
...
SAOKOTHERCON = {
private ["_stuffI","_stuffM","_stuffW","_stuffIa","_stuffMa","_stuffWa","_text","_foreachindex","_curCon","_myDisplay"];
_curCon = if (count _this > 0) then {_this select 0} else {(missionNameSpace getvariable "Wmode")};
if (isNil"_curCon") exitWith {};
_stuffW = [];
_stuffWa = [];
_stuffM = [];
_stuffI = [];
_stuffMa = [];
_stuffIa = [];
if (_curCon != "F" && {!isNull (uniformContainer VariSelectedUnit)}) then {
_stuffI = _stuffI + ((getitemCargo (uniformContainer VariSelectedUnit)) select 0);
_stuffM = _stuffM + ((getMagazineCargo (uniformContainer VariSelectedUnit))select 0);
_stuffW = _stuffW + ((getWeaponCargo (uniformContainer VariSelectedUnit))select 0);
_stuffIa = _stuffIa + ((getitemCargo (uniformContainer VariSelectedUnit)) select 1);
_stuffMa = _stuffMa + ((getMagazineCargo (uniformContainer VariSelectedUnit))select 1);
_stuffWa = _stuffWa + ((getWeaponCargo (uniformContainer VariSelectedUnit))select 1);
};
if (_curCon != "B" && {!isNull (BackPackContainer VariSelectedUnit)}) then {
_stuffI = _stuffI + ((getitemCargo (BackPackContainer VariSelectedUnit))select 0);
_stuffM = _stuffM + ((getMagazineCargo (BackPackContainer VariSelectedUnit))select 0);
_stuffW = _stuffW + ((getWeaponCargo (BackPackContainer VariSelectedUnit))select 0);
_stuffIa = _stuffIa + ((getitemCargo (BackPackContainer VariSelectedUnit))select 1);
_stuffMa = _stuffMa + ((getMagazineCargo (BackPackContainer VariSelectedUnit))select 1);
_stuffWa = _stuffWa + ((getWeaponCargo (BackPackContainer VariSelectedUnit))select 1);
};
if (_curCon != "V" && {!isNull (VestContainer VariSelectedUnit)}) then {
_stuffI = _stuffI + ((getitemCargo (VestContainer VariSelectedUnit))select 0);
_stuffM = _stuffM + ((getMagazineCargo (VestContainer VariSelectedUnit))select 0);
_stuffW = _stuffW + ((getWeaponCargo (VestContainer VariSelectedUnit))select 0);
_stuffIa = _stuffIa + ((getitemCargo (VestContainer VariSelectedUnit))select 1);
_stuffMa = _stuffMa + ((getMagazineCargo (VestContainer VariSelectedUnit))select 1);
_stuffWa = _stuffWa + ((getWeaponCargo (VestContainer VariSelectedUnit))select 1);
};
_text = "In Other Containers<br/>";
{
_text = _text + format ["<img size='1.3' image='%1'/> ",(getText(configfile >> "CfgWeapons" >> _x >> "picture"))]+(getText(configfile >> "CfgWeapons" >> _x >> "displayName"));
if (_stuffWa select _foreachindex > 1) then {_text = _text +(format [" x%1",_stuffWa select _foreachindex]);};
_text = _text +"<br/>";
} foreach _stuffW;
{
_text = _text + format ["<img size='1.3' image='%1'/> ",(getText(configfile >> "CfgWeapons" >> _x >> "picture"))]+(getText(configfile >> "CfgWeapons" >> _x >> "displayName"));
if (_stuffIa select _foreachindex > 1) then {_text = _text +(format [" x%1",_stuffIa select _foreachindex]);};
_text = _text +"<br/>";
} foreach _stuffI;
{
_text = _text + format ["<img size='1.3' image='%1'/> ",(getText(configfile >> "CfgMagazines" >> _x >> "picture"))]+(getText(configfile >> "CfgMagazines" >> _x >> "displayName"));
if (_stuffMa select _foreachindex > 1) then {_text = _text +(format [" x%1",_stuffMa select _foreachindex]);};
_text = _text +"<br/>";
} foreach _stuffM;
disableserialization;
_myDisplay = findDisplay 1992;
