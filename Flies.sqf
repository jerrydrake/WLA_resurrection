...
_flockCount = if (count _this > 2) then {_this select 2} else {_flockArea / 10};
_flockHeight = if (count _this > 3) then {_this select 3} else {0.1};
...
_type = ["DragonFly","HoneyBee","HouseFly","Mosquito","ButterFly_random","DragonFly","HoneyBee","HouseFly","Mosquito","HoneyBee","HouseFly","Mosquito"] call RETURNRANDOM; 


	_crow = _type camcreate [
		(_flockPos select 0) - _flockArea + (random _flockArea)*2 + 30,
		(_flockPos select 1) - _flockArea + (random _flockArea)*2,
		_flockHeight
	];
	[_crow,_wp0,_wp1,_wp2,_wp3,_flockArea] execfsm "CrowsM.fsm";
