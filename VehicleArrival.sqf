//"B_G_Offroad_01_F"
private ["_cVar2","_vClass","_vRoom","_unitsG","_cU","_c","_all","_cars","_driver","_aU","_obj","_veh","_group","_side","_end","_start","_st","_startV","_wp1"];
_group = if (typename (_this select 0) == "ARRAY") then {(_this select 0) select 0} else {_this select 0};
...
_rr = 700;
_ra = 450;
if (typename (_this select 0) == "ARRAY") then {_rr = 1600;_ra = 700;};
_start = [(_end select 0)+_rr-(random _rr*2),(_end select 1)+_rr-(random _rr*2),0];
while {surfaceiswater _start || {vehicle player distance _start < _ra}} do {_start = [(_end select 0)+_rr-(random _rr*2),(_end select 1)+_rr-(random _rr*2),0]; sleep 1;};

_veh = ["B_G_Offroad_01_F",6];
switch _side do {
//VEH ZONE "rhs_tigr_vmf","rhs_tigr_3camo_vmf","rhs_tigr_ffv_3camo_vmf","rhs_tigr_ffv_vmf","rhs_tigr_ffv_vmf","rhs_uaz_open_vmf"
case EAST: {
...
{_x moveincargo _obj;sleep 0.01;} foreach _aU;
sleep 0.01;
} foreach _all;
...
{CARS pushBack _x;_x SPAWN {waitUntil {sleep 2; isNull _this || {count crew _this == 0}};if (!isNull _this) then {[_this,0] SPAWN VehLife;};waitUntil {sleep 2; isNull _this || {count crew _this > 0}};if (!isNull _this) then {[_this,1] SPAWN VehLife;};};sleep 0.01;} foreach _cars;
