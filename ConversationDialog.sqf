...
if ({isNil{_x getvariable "Ghost"}} count (nearestObjects [locationposition (player CALL SAOKNEARESTVIL), ["Land_BarrelWater_F"], 350]) == 0) then {_tasks = _tasks + [["VillageTasks\TaskWater.sqf","Deliver Water"]];};
...
if !((locationposition (player CALL SAOKNEARESTVIL)) distance (getposATL ([locationposition (player CALL SAOKNEARESTVIL)] CALL RETURNGUARDPOST)) < 700) then {_tasks = _tasks + [["VillageTasks\TaskGuardPost.sqf","Guardpost Builder"]];};
