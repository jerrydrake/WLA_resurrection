...
if (random 1 > 0.6) then {FORTRESSESVAR pushback _loc;} else {deletemarker _loc;FORTRESSESMM = FORTRESSESMM - [_loc];};
...
if ({_cPos distance getmarkerpos _x < 400} count FORTRESSESMM == 0) then {
_c = [1,2] call RETURNRANDOM;
for "_i" from 0 to _c do {
_pos = [(_cPos select 0)+200-(random 400),(_cPos select 0)+200-(random 400),0];
_cT = nearestBuilding _pos;
if (count (_cT call SAOKBUILDINGPOS) > 1 && {{_cT distance getmarkerpos _x < 200} count FORTRESSESMM == 0}) then {
if (count FORTRESSESMM > 25) then {
_mar = FORTRESSESMM deleteAt 0;
_mar = FORTRESSESVAR deleteAt 0;
deletemarker _mar;
};
_mar = format ["FORTM%1",NUMM];
NUMM=NUMM+1;
_mar2 = createMarker [_mar,getposATL _cT];
