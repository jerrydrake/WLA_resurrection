...
if (count _this == 0) then {
	...
	_mnpvars = [];
	{if (!isNil{missionNamespace getvariable _x}) then {_mnpvars pushback [_x, missionNamespace getvariable _x];};} foreach ["UIBOXOFF","NoRadioVV","TeDam","ARTYAR","Eridanus","AMBbattles","SUPDEPOTS","CheConf","CIVMODE","CRATECLAS","IFSOVIET","IFENABLED","RHSENABLED","BADBUILDING","GOODBUILDING","PID","NewRespawn","NewFatigue","ComingChapters","SaOkChapters","Autosave","EventLog","RandomEvents","MAXACTIVEV","VarDisableArty","DISVAR","MULTLIFE","GUARDLIM","PNEEDS","DIFLEVEL","AutoReplac","TeamStay","PLSTREGHT","HITEMDIS","CIdentity","CusVoiceOff","TIMMUL","ResTT","SAOKPP","FogOFF","RainyW","DisW","SAOKDYNTASKS","Bumblebeeman","CapturedAll","NORANWEES","SaOkNoAnimals","Bwalll"];
