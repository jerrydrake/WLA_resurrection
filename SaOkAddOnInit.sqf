...
_oldVehAr = ((_mN CALL SAOKZONEDR) CALL APUFF);
_newAr = [];
{
_nV = (_vehAr call RETURNRANDOM);
_newAr pushback [_nV,(_nV CALL SAOKRETVEHAMM),1,0];
} foreach _oldVehAr;
//missionNamespace setvariable [_mN,_newAr];
_mN setvariable ["ZONEDATA",_newAr]; 
};
{
_mType = (_x getvariable "Mtype");
switch (_mType) do {
//SPEEDBOATS
...
case "o_art": {if ((_x getvariable "Mtext")== " AA") then {_n = [_x,ARMEDAA select 1] CALL _f;};};
case "b_art": {if ((_x getvariable "Mtext") == " AA") then {_n = [_x,ARMEDAA select 0] CALL _f;};};
};
} foreach VEHZONESA;
...
lbSetData [1520, _foreachIndex, _x];
...
disableserialization;
_myDisplay = findDisplay 2004;
_control = _myDisplay displayCtrl 1520;
_item = _control lbData _classN; 
