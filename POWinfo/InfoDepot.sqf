...
_real = true;
_st = [getposATL player, 3000,"(1 - sea) * (1 + meadow)* (1 - hills)",""] CALL FUNKTIO_POS;
_home = (_st select 0) select 0;
_d = 3000;
while {player distance _home < 60} do {
_d = _d + 400;
_st = [getposATL player, _d,"(1 - sea) * (1 + meadow)* (1 - hills)",""] CALL FUNKTIO_POS;
_home = (_st select 0) select 0;
sleep 0.1;
};
...
[_someId, "onEachFrame", {
	...
	drawIcon3D ["\A3\ui_f\data\map\markers\military\unknown_CA.paa", ICONCOLORGREEN, _this,1.51, 1.51, 0, (format ["Find CSAT Depot: %1m",round (_this distance player)]), 1, SAOKFSI, "TahomaB"];
}, _mP] call BIS_fnc_addStackedEventHandler;
_ranCrate = (CRATECLAS select 1) call RETURNRANDOM; 
[_home,40] SPAWN CreateDepot;
waitUntil {sleep 10; player distance _home < 50};
