...
_CHOPDOWN = {
...

	{
		...
		_nul = [_x] SPAWN {
			...
			waituntil {
				sleep 10;
				vehicle player distance _unit > 700 || {
					isNull _unit
				}
			};
	...
	"Chopper pick up can be called again"
	SPAWN HINTSAOK;
...
_EXIT = {
	...
	sleep 7;
	if (typeOf(_chop select 0) != "MH6J_EP1") then {
		_nul = [(_chop select 2), 1, 1, group(_this select 0)] SPAWN FSlowChopperSupportF;
	};
	CHOPAVAIL = true;
	"Chopper pick up can be called again"
	SPAWN HINTSAOK; {
...
//CONDITIONS
_CONDCHOPDOWN = {
	private["_bool"];_bool = false;
	if (!(alive(_this select 0)) || {!(alive driver(_this select 0))
		} || {
			{
				alive _x
			}
			count crew(_this select 0) == 0
		}) then {
		_bool = true;
	};_bool
};
_CONDPLAYERLEFT = {
	private["_bool"];_bool = false;
	if (_this distance vehicle player > 600) then {
		_bool = true;
	};_bool
};
_CONDPLAYEROUT = {
...
if (!isNil "RHSENABLED" || {!isNil "Eridanus"
	}) then {
	_chopclass = AIRCARRIERCHOP select 0;
};
...
waitUntil {
	sleep 3;
	(_chop select 0) distance _LZ < 200 || {
		_chop CALL _CONDCHOPDOWN
	} || {
		((_this select 0) in (_chop select 0))
	}
};
...
if (!isNil {
		player getvariable "SmokeTime"
	}) then {
	_LZ = vehicle(_this select 0);
	_accu = 50;
	leader(_chop select 2) sidechat "Eagle to Wolf, I think I see you guys with the smoke. Landing in notime. Out";
}
else {
	leader(_chop select 2) sidechat "Eagle to Wolf, I cant see you guys but I am landing now. I hope you are close. I dont want to wait long. Out";
};

// Find a suitable area -- Disabled
_ok = true;
_obj = objNull;
while {
	_ok
...
do {
	...
	_max = 0;
	while {
		(!([_start2, 10] call SAOKISFLAT) || {
			[_start2, _chopclass] CALL SAOKTAKENBOX
		}) && {
			_max < 40
		}
	}
	...
	/*
	// Find a suitable area -- Second version
	_start2 = [_LZPos, 0,_accu,10,0,0.2,0,[],[[0,0],[0,0]]] call BIS_fnc_findSafePos; // find a valid pos
	if (0 == (_start2 select 0) && 0 == (_start2 select 1)) then {
		leader (_chop select 2) sidechat "Eagle to Wolf, I can't find a suitable LZ. Out";
		_start2 = [_LZPos, 0,_retryAccu,10,0,0.2,0,[],[[0,0],[0,0]]] call BIS_fnc_findSafePos; // find a valid pos
	};
	*/

	_start2 = _LZPos; // Disabling the finding above
	...
	waitUntil {
		sleep 3;
		unitready leader(_chop select 2) || {
			_LZ CALL _CONDPLAYERLEFT
		} || {
			_chop CALL _CONDCHOPDOWN
		} || {
			((_this select 0) in (_chop select 0))
	...
	waitUntil {
		sleep 2;
		getposATL(_chop select 0) select 2 < 4 || {
			isNull(_chop select 0)
		} || {
			((_this select 0) in (_chop select 0))
		} || {
			_LZ CALL _CONDPLAYERLEFT
	...
	waitUntil {
		sleep 3;
		getposATL(_chop select 0) select 2 > 5 || {
			isNull(_chop select 0)
		} || {
			(((_this select 0) in (_chop select 0)) && {
				{
					alive _x && {
						...
						_x distance(_this select 0) < 100
					}
				}
				count units group(_this select 0) == 0
			})
		} || {
			_LZ CALL _CONDPLAYERLEFT
		}
	};
	if (getposATL(_chop select 0) select 2 > 5) then {
		deletevehicle _obj;
	} else {
		_ok = false;
	};
...
//LEAVE
deletevehicle _obj;
"Click on map for LZ-point - changeable until landing"
SPAWN HINTSAOK;
...
waitUntil {
	sleep 2;
	getposATL(_chop select 0) select 2 > 10 || {
		isNull(_chop select 0)
	} || {
		_chop CALL _CONDCHOPDOWN
	} || {
		_chop CALL _CONDPLAYEROUT
	}
};
if (_chop CALL _CONDCHOPDOWN) exitWith {
	//CHOPPER DOWN
...
waitUntil {
	sleep 2;
	(getposATL(_chop select 0) select 2) < 1.5 || {
		isNull(_chop select 0)
	} || {
		_chop CALL _CONDCHOPDOWN
	} || {
