...
if (alive (driver (vehicle leader _chopg))) then {_nul = [_chopg, [1036.67,7113.46,0], 7] SPAWN FMoveAndDelete;} else {_nul = [_chopg] SPAWN FMoveAwayAndGetDeleted; CARS = CARS + [(vehicle leader _chopg)];};
