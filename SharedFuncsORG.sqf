SAOKREPACKAMMO = {
private ["_m","_co","_f","_tA","_c","_magazinesAmmoFull","_anim44","_num44","_select44"];
...
NEARESTLOCATIONNAME = {
_t = if (!isnil"_this" && {typename _this == "OBJECT" || {typename _this == "ARRAY" && (count _this > 1)}}) then {_this} else {[] CALL SAOKSORTPLAYER};
...
if (_mes == "") exitWith {};
if (visiblemap || {count _this > 2}) then {gameLogic1 globalradio _mes;};
};
...
SAOKVIRSOUND = {
private ["_bb","_gunSel","_as","_s","_wepp","_arGs","_p2","_ammoCon","_p","_oka","_vehc","_ammoc","_typ"];
...
if (count _v > 0) then {
_v = [_v,[],{player distance _x},"ASCEND"] call SAOKSORTBY;
...
HINTSAOK = {
	private ["_n"];
	if (typename _this == "STRING" && {_this != ""}) then {SAOKHINTTEE pushback (toUPPER _this);};
	...
	if !(count (missionnamespace getvariable "SAOKHINTTEE") > 0) exitWith {};
		if (count ((missionnamespace getvariable "SAOKHINTTEE") select 0) != 1) exitWith {
		_n = (missionnamespace getvariable "SAOKHINTTEE");
		_n deleteAT 0;
		missionnamespace setvariable ["SAOKHINTTEE",_n];
		};
	_h = ((missionnamespace getvariable "SAOKHINTTEE") select 0);
	if ({isNil"_x"} count [_h] > 0 || {typename _h != "STRING"}) exitWith {
	_n = (missionnamespace getvariable "SAOKHINTTEE");
	_n deleteAT 0;
	missionnamespace setvariable ["SAOKHINTTEE",_n];
	};
	 _pic0 = findDisplay 46 ctrlCreate ["RscBackground", -1];
	 _pic0 ctrlSetBackgroundColor [0.2,0.6,0.4,0.7];
     _pic0 ctrlSetPosition [safeZoneX,(safeZoneY + safeZoneH)*0.55 - safeZoneH*0.10,0,0];
     _pic0 ctrlCommit 0;
	 _pic0 ctrlSetPosition [safeZoneX,(safeZoneY + safeZoneH)*0.55 - safeZoneH*0.10,safeZoneW*0.2,safeZoneH*0.08];
	 _pic0 ctrlCommit 1;
     sleep 1;
	 _pic3 = findDisplay 46 ctrlCreate ["RscStructuredText", -1];
	 _pic3 ctrlSetText _h;
	 //_pic2 ctrlSetTextColor [0.6,0,0.4,0.7];
     _pic3 ctrlSetPosition [safeZoneX,(safeZoneY + safeZoneH)*0.55 - safeZoneH*0.10,0,0];
     _pic3 ctrlCommit 0;
	 _pic3 ctrlSetPosition [safeZoneX,(safeZoneY + safeZoneH)*0.55 - safeZoneH*0.10,safeZoneW*0.2,safeZoneH*0.08];
	 _pic3 ctrlCommit 1;
	 sleep 6.5;
	 _pic3 ctrlSetPosition [safeZoneX,(safeZoneY + safeZoneH)*0.55 - safeZoneH*0.10,0,0];
     _pic3 ctrlCommit 1;
