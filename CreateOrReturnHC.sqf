...
if (count(_this select 0) > 0)
	then
	{
		...
		{
			(typeof(vehicle player)iskindof "Air")
		}
		)
			then
			{
				"You cant create HC group when you or any of the selected team-mates is in chopper or plane" SPAWN HINTSAOK;
			}
		else
		{
			...
			{
				{
					group(driver(vehicle _x)) == group _x
				}
				count(_this select 0) > 0
			}
			)
...
//RETURN HC
if (count(_this select 1) > 0)
	then
	{
		{
			if !(typeof(vehicle leader _x)iskindof "Air" ||
			{
				typeof(vehicle leader _x)iskindof "UGV_01_base_F"
			}
			)
				then
				{
