...
_cl = "";
switch (_type) do {
case "Cargo Net": {_cl = "B_CargoNet_01_ammo_F";};
...
if (_cl == "") then {
{if (getText (configfile >> "CfgVehicles" >> _x >> "displayName") == _type) then {_cl = _x;};} foreach ["I_G_Van_01_transport_F"] + (ARMEDVEHICLES select 0)+(ARMEDCARRIER select 0);
};
...
if (_cl iskindof "ReammoBox_F" || {true} || {!isNil"IFENABLED"}) then {
