...
while {surfaceisWater _start2 || {!([_start2,40] CALL SAOKISFLAT)} ||  {vehicle player distance _start2 < 500} || {count (_start2 nearRoads 40) > 0} || {!(_start2 CALL SAOKINMAP)}} do {
sleep 0.1;
_d = _d + 300;
...
if (count _this > 0) then {_start2 = _this select 0;};
CurTaskS = CurTaskS - ["MainTasks\Task1_ConvertResistance.sqf"];
CurTaskS = CurTaskS + [["MainTasks\Task1_ConvertResistance.sqf",_start2]];
...
_eOc = if (isNil"IFENABLED") then {["O_G_Offroad_01_armed_F","O_G_Offroad_01_armed_F"]} else {["Lib_SdKfz251_captuRed","Lib_SdKfz251_captuRed"]};
if (count _this == 0) then {_n = [_start3, "ColorRed",_eOc] CALL AddVehicleZone;};
sleep 1;
_start3 = [_start2, 200,0,"(1 - sea) * (1 + meadow)",300] CALL SAOKSEEKPOS;
_eOc = if (isNil"IFENABLED") then {["O_G_Offroad_01_armed_F","O_G_Offroad_01_armed_F"]} else {["Lib_SdKfz251_captuRed","Lib_SdKfz251_captuRed"]};
if (count _this == 0) then {_n2 = [_start3, "ColorRed",_eOc] CALL AddVehicleZone;};
sleep 1;
if (count _this == 0) then {_nul = [_start2,"RES",(15+random 25)] SPAWN CreateRoadBlock;};
_eOc = if (isNil"IFENABLED") then {"O_G_officer_F"} else {"LIB_SOV_captain"};
if (!isNil"Eridanus") then {_eOc = "TEI_Ins_URF_Officer";};
if (!isNil"CheConf") then {_eOc = "O_mas_cer_Soldier_off_F";};
_group = [_start2, EAST, [_eOc],[],[],[0.4,0.8]] call SpawnGroupCustom;
...
waitUntil {sleep 10; isnull _t || {!alive _t} || {_t in (crew _c)} || {!isNil{_t getvariable "SaOkSurrendeRed"}}};
if (_t in (crew _c)) then {_t allowfleeing 1;"Target is trying to flee with vehicle" SPAWN HINTSAOK;_t setspeedmode "NORMAL";_t setbehaviour "AWARE";};
};
};
...
waitUntil {sleep 10; isNull _crowKing || {!alive _crowKing} || {!isNil{_crowKing getvariable "SaOkSurrendeRed"}}};
