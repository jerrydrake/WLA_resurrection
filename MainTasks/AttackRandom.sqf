...
_desc =("Friendlies are about to attack on enemy camp. Should we assists or stay back?"+_ResultColor);
[
	WEST, // Task owner(s)
...
if (player distance _locationA < 1400 || {player distance _start2 < 600}) then {
	...
	if ({alive _x && {!(fleeing _x)}} count (units _group)  + (units (_tg1 select 2)) < 2 || {_timer < time}) then {
		...
		_Lna = _locationA CALL NEARESTLOCATIONNAME;
		_header = format ["Operation Against enemy Camp near %1 Failed",_Lna];
		...
		CantCommand = CantCommand + [_group];
		_Lna = _locationA CALL NEARESTLOCATIONNAME;
		_header = format ["Bloody Operation Against enemy Camp near %1 Ended With Green Victory. Locals Celebrating On Streets",_Lna];
		[_header, date] CALL SAOKEVENTLOG;
