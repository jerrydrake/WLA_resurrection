...
_c = "";
_p = SAOKMAPSIZE*0.5;
{if (typename _c == "STRING" || {locationposition _x distance player > locationposition _c distance player}) then {_c = _x};} foreach (nearestLocations [[_p,_p,0], ["CampR"], SAOKMAPSIZE*0.7]);
_start = if (_c != "") then {locationposition _c} else {[(vehicle player),6000,2000,"(1 + meadow) * (1 - sea)"] CALL SAOKSEEKPOS};
_r = 200;
_roads = (_start nearRoads _r);
while {count _roads == 0} do {_r = _r + 40;_roads = (_start nearRoads _r);};
...
[
WEST, // Task owner(s)
"taskF1", // Task ID (used when setting task state, destination or description later)
...
_vz = "";
_vz = if (count _this == 0 || {!((_this select 0) in VEHZONESA)}) then {[_start, "ColorRed",_ar] CALL AddVehicleZone} else {_this select 0};
...
while {getmarkercolor _p == "ColorGreen" && {{getmarkercolor _x != "ColorGreen"} count PierMarkers > 4}} do {_p = PierMarkers call RETURNRANDOM;};
_wp = [getmarkerpos _p,100,0,"(1 - sea)"] CALL SAOKSEEKPOS;  
[_vz,[],_wp] SPAWN SAOKFINDROUTEANDMOVE;
while {_vz in VEHZONESA && {(_vz getvariable "Mcolor") == "ColorRed"}} do {if (locationposition _vz distance [0,0,0] > 100 && {!surfaceisWater (locationposition _vz)}) then {_start = locationposition _vz;};sleep 4;};
_nul = ["taskF1","SUCCEEDED"] call SAOKCOTASK;
[_someId, "onEachFrame"] call BIS_fnc_removeStackedEventHandler;
SZONES = SZONES - [_vz];
CurTaskS = CurTaskS - [["maintasks\CaptureOfficer.sqf",_vz]];
} else {_start = _this select 0;};
...
if (isClass(configFile >> "cfgVehicles" >> "rhs_msv_officer")) then {_eOc = "rhs_msv_officer";};
if (!isNil"CheConf") then {_eOc = "O_mas_cer_Soldier_off_F";};
_off = leader ([_start, EAST, [_eOc],[],[],[0.4,0.8]] call SpawnGroupCustom);
...
[
WEST, // Task owner(s)
"taskF2", // Task ID (used when setting task state, destination or description later)
...
waitUntil {sleep 5; player distance _start < 30 || {player distance _off < 30}};
_n = ["taskF1"] CALL BIS_fnc_deleteTask;
_nul = ["taskF2","SUCCEEDED"] call SAOKCOTASK;
[_someId, "onEachFrame"] call BIS_fnc_removeStackedEventHandler;
_b = false; 
if (player knowsabout _off > 1 || {[_off, player] CALL FUNKTIO_LOS}) then {_b = true;};
...
[
WEST, // Task owner(s)
"taskF3", // Task ID (used when setting task state, destination or description later)
_h, // Task description
objNull, // Task destination
true // true to set task as current upon creation
] call SAOKCRTASK;
...
if (!_b) then {
_aika = time + 60;
waitUntil {sleep 3; _aika < time || {!isNil{_off getvariable "SaOkSurrendeRed"}}};
if (_aika < time) then {
...
waitUntil {sleep 3; isNull _off || {!isNil{_off getvariable "SaOkSurrendeRed"}}};
_n = ["taskF2"] CALL BIS_fnc_deleteTask;
_nul = ["taskF3","SUCCEEDED"] call SAOKCOTASK;
[_someId, "onEachFrame"] call BIS_fnc_removeStackedEventHandler;
//dostop _off;
_off disableAI "move";
_n = [
...
waitUntil {sleep 3; isNull _off || {vehicle _off != _off} || {_aika < time}};
DONTDELGROUPS = DONTDELGROUPS - [_off];
if (_aika < time) then {_off SPAWN SAOKDELINBLACK;};
gameLogic1 globalchat localize "STR_milT3_l8";
_n = ["taskF3"] CALL BIS_fnc_deleteTask;
CurTaskS = CurTaskS - [["maintasks\CaptureOfficer.sqf",_start]];
_nul = ["taskF4","SUCCEEDED"] call SAOKCOTASK;
CapturedAll = CapturedAll + 1;
sleep 20;
_n = ["taskF4"] CALL BIS_fnc_deleteTask;
