...
while {
	surfaceisWater _start || {!([_start, 40] CALL SAOKISFLAT)
	} || {!(_start CALL SAOKINMAP)
	} || {
		count(_start nearRoads 40) > 0
	} || {
		(nearestbuilding _start) distance _start < 50
	}
}
do {
	sleep 0.1;
...
waitUntil {
	sleep 1;
	!isNil {
		missionnamespace getvariable "PowLoc"
	} && {!isNil {
			missionnamespace getvariable "Pris"
		}
	}
};
...
_d = 80;
while {
	_start2 distance _start < 60 || {!([_start2, 20] CALL SAOKISFLAT)
	}
}
do {
	_d = _d + 10;
...
waitUntil {
	sleep 4;
	vehicle player distance _start < 250
};
...
if (vehicle player distance _animal < 40) then {
	...
	waitUntil {
		sleep 4;
		vehicle player distance _animal < 15 && {
			([_animal, player] CALL FUNKTIO_LOS || {
				_animal CALL _ff
			})
		} && {
			({
	...
	//deleteMarker _marker;
	_nul = ["taskRR", "FAILED", true] call BIS_fnc_taskSetState;
};
_nul = [4] SPAWN MusicT;
DONTDELGROUPS = DONTDELGROUPS - [_group]; {
	DONTDELGROUPS = DONTDELGROUPS - [_x];
	_nul = [leader _x, ""] SPAWN FHideAndDelete;
}
foreach _ggg;
