...
if (isNil"LineSelected" || {getmarkercolor ((["ColorRed"] CALL NEARESTCAMP) getvariable "Marker") != "ColorRed"} || {surfaceisWater (locationposition (["ColorRed"] CALL NEARESTCAMP))}) exitWith {};
...
waitUntil {sleep 6; !alive _civ || {_civ distance vehicle player < 20}};
_civ enableAI "MOVE";
_civ setspeedmode "FULL";
_civ setbehaviour "AWARE";
if (vehicle player == player && {alive _civ}) then {
_n = [
[getposATL player, player, _civ],
[],
[
[[[_civ], ["Thank you, I though I was dead"],5]]
...
//TASK SUCCESSFULL
waitUntil {sleep 6; !alive _civ || {_civ distance (locationposition (["ColorRed"] CALL NEARESTCAMP)) > 50}};
DONTDELGROUPS = DONTDELGROUPS - [_group];
_str = "Task"; 
